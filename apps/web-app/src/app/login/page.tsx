...
// Real login: Google sign-in popup → ID token → exchanged for an httpOnly session
// cookie via POST /api/auth/session → into the app. The (app) layout then verifies
// server-side and routes zero-grant users to /awaiting-access. Restrained Refined-Navy
// surface; Bahasa Indonesia. (Replaces the demo persona picker.)
export default function LoginPage() {
  // useSearchParams needs a Suspense boundary (it opts the subtree out of static render).
  return (
    <Suspense>
      <LoginForm />
    </Suspense>
...
function LoginForm() {
  ...
  // Set by /api/auth/logout when a stale session cookie was cleared (see proxy loop fix).
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') {
        setError(null) // user dismissed the popup — no error to show
      } else {
        setError((e as Error).message || 'Terjadi kesalahan saat masuk.')
      }
      setPending(false)
    }
  }
    ...
    <main className="flex min-h-screen items-center justify-center bg-muted/30 px-4 py-10">
      <div className="w-full max-w-sm space-y-7">
        <header className="flex flex-col items-center text-center">
          <MizanIcon className="size-14" />
          <h1 className="mt-4 font-heading text-2xl font-semibold tracking-[0.12em] text-primary">MIZAN</h1>
          <p className="mt-1 text-sm text-muted-foreground">Sistem Originasi Pembiayaan — Hijra Bank</p>
        </header>

        <Card>
          <CardContent className="flex flex-col gap-5 px-6 py-8">
            <div className="space-y-1 text-center">
              <h2 className="font-heading text-base font-medium">Masuk ke akun Anda</h2>
              <p className="text-sm text-muted-foreground">
                Gunakan akun Google kantor Anda untuk melanjutkan.
              </p>
            </div>
            ...
            {error && (
              <div
                role="alert"
                className="flex items-start gap-2 rounded-lg bg-danger-subtle px-3 py-2 text-sm text-danger-foreground"
              >
            ...
            >
              {pending ? <Loader2 className="size-4 animate-spin" aria-hidden="true" /> : <GoogleIcon />}
              {pending ? 'Memproses…' : 'Masuk dengan Google'}
            </Button>
          </CardContent>
        </Card>

        <p className="text-center text-xs text-muted-foreground">
          PT BPRS Hijra Alami — akses terbatas untuk pengguna berwenang
        </p>
      </div>
    </main>
  )
